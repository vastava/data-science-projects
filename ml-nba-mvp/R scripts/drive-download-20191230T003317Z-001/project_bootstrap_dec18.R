...
boot_osr <- function(data, index) {
  ...
  return(1 - (sum((labels - predictions)^2)/sum((labels - mean_obs)^2)))
}
...
##stepwise lin reg
stepwise_df = data.frame(labels = nba_csv.train$award_share, predictions =  pred.bswr)
set.seed(342)
...
##random forest
rf_df = data.frame(labels = nba_csv.train$award_share, predictions = pred.best.rf)
set.seed(6722)
...
##boosting
boost_df = data.frame(labels = nba_csv.train$award_share, predictions = pred.best.boost)
set.seed(9391)
